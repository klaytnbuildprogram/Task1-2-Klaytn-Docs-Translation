...
The sender's balance decreases by the remaining transaction fee. e.g., If the feeRatio is 30, 30% of the fee will be paid by the fee payer, and the remaining 70% of the fee will be paid by the sender.
...
A unit price of gas in peb the sender will pay for a transaction fee. The amount of transaction fee is calculated as gas * gasPrice. For example, if the transaction consumes 10 units of gas and gasPrice is 10^18, the transaction fee will be 10 KLAY. Unit of KLAY is described here.
...
The sender's balance decreases by the remaining transaction fee. e.g., If the feeRatio is 30, 30% of the fee will be paid by the fee payer, and the remaining 70% of the fee will be paid by the sender.k
...
Transactions sent from the account afterward will be validated with this key, once this transaction is executed.
...
The type of TxTypeAccountUpdate. This must be 0x22.
...
The type of TxTypeCancel. This must be 0x3a.
